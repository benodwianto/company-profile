                            ...
                            <?php if ($result->num_rows > 0) : ?>
                                <?php while ($row = $result->fetch_assoc()) : ?>
                                    <div class="product-card">
                                        <img src="../../assets/images/produk/<?= htmlspecialchars(basename($row['foto'])); ?>" alt="<?= htmlspecialchars($row['jenis_sapi']); ?>" class="product-image">
                                        <h3><?= htmlspecialchars($row['jenis_sapi']); ?></h3>
                                        <div class="product-icons">
                                            <a href="../produk/update_produk.php?id=<?= htmlspecialchars($row['id']); ?>" class="icon-link"><i class="fa fa-edit"></i></a>
                                            <a href="../produk/delete_produk.php?id=<?= htmlspecialchars($row['id']); ?>" class="icon-link" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fa fa-trash"></i></a>
                                        </div>
                                    </div>
                                <?php endwhile; ?>
                            <?php else : ?>
